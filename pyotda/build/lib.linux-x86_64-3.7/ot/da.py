...
def sinkhorn_knopp(a, b, M, reg, u=None, v=None, numItermax=1000,
    ...
    if nbb:  # return only loss
        ...
        if log:
            return u.reshape((-1, 1)) * K * v.reshape((1, -1)), u.reshape((-1, 1))[:, 0], v.reshape((-1, 1))[:, 0], log
        else:
            return u.reshape((-1, 1)) * K * v.reshape((1, -1)), u.reshape((-1, 1)), v.reshape((-1, 1))


def sinkhorn_ML(Xs, ys, Xt, a, labels_a, b, M, reg, reg_e, type_reg, reg_cl=0.1, reg_l=1., numItermax=10,
                numInnerItermax_grad=200, numInnerItermax_sink=200, stopInnerThr=1e-9, verbose=False,
                log=False, which_norm="LLt_only_on_Xt", initialisation_L="identity", ML_init=False, margin=1,
                sinkhorn_type="sinkhorn"):
    # print(a, labels_a, b, M, reg, reg_e, type_reg, reg_cl, reg_l, numItermax,
    #       numInnerItermax, stopInnerThr, verbose,
    #       log, which_norm, initialisation_L, balance, margin,
    #       learning_rate)
    """
    ...
    if Xt.shape[1] < 100:
        ...
        mini_batch_size = 15
    LMMN = LargeMarginNearestNeighbor(Xs=Xs, Xt=Xt, y=ys,
                                      nFtsOut=int(initialisation_L),
    ...
    for cpt in range(numItermax):
        if which_norm == "malhanobis":
            C = euclidean_distances(Xs @ np.transpose(LMMN.L_), Xt @ np.transpose(LMMN.L_), squared=False)
            # C = dist(x1=Xs @ np.transpose(LMMN.L_), x2=Xt @ np.transpose(LMMN.L_), metric='sqeuclidean')
        elif which_norm == "L_only_on_Xs":
            C = euclidean_distances(Xs @ np.transpose(LMMN.L_), Xt, squared=False)
            # C = dist(x1=Xs @ np.transpose(LMMN.L_), x2=Xt, metric='sqeuclidean')
        C = cost_normalization(C=C, norm="median")
        if sinkhorn_type == "sinkhorn_class":
            transp = sinkhorn_lpl1_mm(a=a, labels_a=ys, b=b, M=C, reg=reg_e, eta=reg_cl,
                                      ...
                                      verbose=True,
                                      log=False)
        elif sinkhorn_type == "sinkhorn_1":
            if cpt > 0:
                transp, u, v, log = sinkhorn_knopp(a, b, C, reg_e, u=u, v=v,
                                                   numItermax=numInnerItermax_sink,
                                                   stopThr=stopInnerThr,
                                                   log=True)
            else:
                transp, u, v, log = sinkhorn_knopp(a, b, C, reg_e,
                                                   numItermax=numInnerItermax_sink,
                                                   stopThr=stopInnerThr,
                                                   log=True)
        elif sinkhorn_type == "sinkhorn":
            transp = sinkhorn(a, b, C, reg_e,
                              numItermax=numInnerItermax_sink,
                              stopThr=stopInnerThr,
                              log=False)

        if verbose:
            print("Modification after sinkhorn", cpt)
            LMMN.transp = transp
            print("cout de la loss", LMMN._loss_grad(LMMN.L_)[0])
        if cpt < numItermax -1:
            LMMN.fit(transp=transp)
        if verbose:
            # L2 = cost_normalization(C=LMMN.L_, norm="median")
            print("Modification after LMMN", cpt)
            print("cout de la loss", LMMN._loss_grad(LMMN.L_)[0])
        # print(cpt, LMMN._loss_grad(LMMN.L_)[0])
        # new_loss = LMMN._loss_grad(LMMN.L_)[0]
        # if abs(old_loss - new_loss) < 10**(-4):
        #     break
        # old_loss = new_loss

        # if verbose:
        #     print("L", cpt, np.round(np.transpose(LMMN.L_)[:10, :10], 3), np.mean((LMMN.L_)**2))
        #     e = np.array([i * int(len(ys)/20) for i in range(20)])
        #     print("C", cpt, np.round(C[e, :12], 3), np.mean(C**2))

    return transp, np.transpose(LMMN.L_)
...
class SinkhornMLTransport(BaseTransport):
    ...
    def __init__(self, reg_e=1., reg_cl=0.1, reg=0.1, reg_l=1., type_reg="l1",
                 max_iter=10,
                 max_inner_iter_sink=200,
                 max_inner_iter_grad=200,
                 log=False,
                 tol=10e-9,
                 verbose=False,
                 metric="sqeuclidean", norm=None,
                 which_norm="LLt_only_on_Xt",
                 out_of_sample_map='ferradans',
                 limit_max=np.infty,
                 initialisation_L="identity",
                 ML_init=False,
                 margin=1,
                 sinkhorn_type="sinkhorn"):
        ...
        self.out_of_sample_map = out_of_sample_map
        self.limit_max = limit_max
        self.initialisation_L = initialisation_L
    ...
    def fit(self, Xs, ys=None, Xt=None, yt=None, distribution_estimation=None):
        ...
        if check_params(Xs=Xs, Xt=Xt, ys=ys):
            super(SinkhornMLTransport, self).fit(Xs, ys, Xt, yt, distribution_estimation)
            ...
            returned_ = sinkhorn_ML(
                ...
                initialisation_L=self.initialisation_L,
                ML_init=self.ML_init,
                margin=self.margin,
                numInnerItermax_sink=self.max_inner_iter_sink,
                sinkhorn_type=self.sinkhorn_type
            )

        # deal with the value of log
        if self.log:
            self.coupling_, self.L, self.log_ = returned_
        else:
            self.coupling_, self.L = returned_
            self.log_ = dict()
        return self
    ...
    def transform_both(self, Xs, Xt, batch_size=126):
        ...
        if check_params(Xs=Xs):
            ...
            if self.which_norm == "malhanobis":
                return transp_Xs @ self.L, Xt @ self.L, Xs @ self.L, Xt @ self.L
            elif self.which_norm == "L_only_on_Xt":
                return transp_Xs @ self.L, Xt @ self.L, Xs, Xt @ self.L
            elif self.which_norm == "L_only_on_Xs":
                return transp_Xs, Xt, Xs @ self.L, Xt
            elif self.which_norm == "LLt_only_on_Xt":
                return transp_Xs @ self.L @ np.transpose(self.L), Xt @ self.L @ np.transpose(
                    self.L), Xs, Xt @ self.L @ np.transpose(self.L)
