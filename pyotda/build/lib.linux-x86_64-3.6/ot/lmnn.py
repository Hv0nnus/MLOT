...
def creat_mini_batch(ys, mini_batch_size, Xt_len):
    ...
    # print("Xt_len", Xt_len)
    # print("np.min(counts)", np.min(counts))
    # print("len(ys)", len(ys))
    # print("mini_batch_size",mini_batch_size)
    # print("int(len(ys) / mini_batch_size)", int(len(ys) / mini_batch_size))
...
def gradient_L(gamma, L, Xs, Xt, which_norm="malhanobis"):
    # ONLY L_ONLY_ON_XS IS EFFICIENT !!!!
    time1 = time.time()
    if which_norm == "malhanobis":
        ...
        # print("5.5", time.time() - time1)
        # gradient_transport = np.sum(gradient_transport, axis=(0, 1))
        # print("5.6", time.time() - time1)
        # print("to see if we got the same value", gradient_transport)
        # print("second try")
        time1 = time.time()
        # print("6.1", time.time() - time1)
        gamma_i = np.sum(gamma, axis=1)
        # print("6.2", time.time() - time1)
        Xs_gamma_i = Xs * np.expand_dims(gamma_i, 1)
        # print("6.3", time.time() - time1)
        first_term = L @ Xs_gamma_i.T @ Xs
        # print("6.4", time.time() - time1)
        second_term = Xs.T @ gamma @ Xt
        # print("6.5", time.time() - time1)
        gradient_transport = first_term - second_term
        # print("6.6", time.time() - time1)
        # print("to see if we got the same value", gradient_transport_)
...
class LargeMarginNearestNeighbor():
    def __init__(self, Xt, Xs, y,
        ...
        if ML_init == "PCA":
            ...
            # print("ML_init == SS")
            # print("xs", XS)
            # print("np.transpose(XS @ np.transpose(XS))", self.L_)
            self.nFtsOut_ = np.shape(self.L_)[0]
    ...
    def _loss_grad(self, L):
        ...
        if self.reg_l < 100:
            ...
            # print("number of batch", len(mini_batch_list[1]))
