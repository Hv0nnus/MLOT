...
# This code in based on the initial code of the following paper. The Optimal Transport term and it's derivative is
# added.
...
def creat_mini_batch(ys, mini_batch_size, Xt_len):
    ...
    mini_batch_size = min(mini_batch_size, len(ys))
    ...
    # If we want to use small part of Xt
...
def gradient_L(gamma, L, Xs, Xt):
    gamma_i = np.sum(gamma, axis=1)
    Xs_gamma_i = Xs * np.expand_dims(gamma_i, 1)
    first_term = L @ Xs_gamma_i.T @ Xs
    second_term = Xs.T @ gamma @ Xt
    gradient_transport = first_term - second_term
...
class LargeMarginNearestNeighbor():
    def __init__(self, Xt, Xs, y,
        ...
        # -------------- How to init L ---------------
        if ML_init == "PCA":
            pcaS = sklearn.decomposition.PCA(max(self.nFtsOut_//2, self.Xs.shape[1]//2), svd_solver=svd_solver).fit(self.Xs)
            pcaT = sklearn.decomposition.PCA(max(self.nFtsOut_//2, self.Xt.shape[1]//2), svd_solver=svd_solver).fit(self.Xt)
            ...
            pcaS = sklearn.decomposition.PCA(self.nFtsOut_, svd_solver=svd_solver).fit(self.Xs)
            pcaT = sklearn.decomposition.PCA(self.nFtsOut_, svd_solver=svd_solver).fit(self.Xt)
    ...
    def _loss_grad(self, L):
        ...
        if self.reg_l < 100:
            ...
            for index_mini_batch in range(len(mini_batch_list[1])):
                ...
                break  # If we don't want to use all mini batch.
